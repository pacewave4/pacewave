...
function renderFeatured(){ 
  ...
  autoScroll(wrap, 3000); // 3 seconds
}
function renderNew(){ 
...
function autoScroll(el, ms){
  let pos = 0;
  const step = 216; // card width 200 + gap 16
  setInterval(()=>{
    const max = el.scrollWidth - el.clientWidth;
    pos += step;
    if(pos > max) pos = 0; // loop back to start
...
function createFloatingBooks(){
  ...
  for(let i=0;i<10;i++){
    ...
    const x = Math.random() * (wrap.clientWidth - 60);
    const y = Math.random() * (wrap.clientHeight - 80);
    div.style.left = x + 'px';
    div.style.top = y + 'px';
    ...
    tiles.push({
      el: div,
      x, y,
      vx: (Math.random() - 0.5) * 0.4, // random velocity x
      vy: (Math.random() - 0.5) * 0.4, // random velocity y
      rot: Math.random() * 360,
      vr: (Math.random() - 0.5) * 0.2  // random rotation speed
    });
  }
  ...
  function animate(){
    ...
    tiles.forEach(t => {
      ...
      // Bounce off edges
      if(t.x < 0 || t.x > w - 60) t.vx *= -1;
      if(t.y < 0 || t.y > h - 80) t.vy *= -1;
